...
BOOL CXlSimpleAutomation::ExlInvoke( IDispatch* i_pdisp, LPOLESTR i_szName, VARIANTARG* i_pvargResult, WORD i_wInvokeAction, WORD i_wFlags )
{
	...
	if (i_pdisp !=nullptr)
	{
		...
		int iwInvokeAction = (INT)CConvertMng::ConvertSigned(CConvertMng::GetCalcBitWise((UINT32)i_wInvokeAction, (INT32)(DISPATCH_PROPERTYPUT | DISPATCH_PROPERTYPUTREF), "&"));
		...
		int iFlags = (INT)CConvertMng::ConvertSigned(CConvertMng::GetCalcBitWise((UINT32)CConvertMng::ConvertSigned(i_wFlags), (INT32)DISP_FREEARGS, "&"));
		if ( iFlags !=0)
		{
			ClearAllArgs();
		}
...
void CXlSimpleAutomation::ClearAllArgs()
{
	...
	{
		int nFlags = (INT)CConvertMng::ConvertSigned(CConvertMng::GetCalcBitWise((UINT32)CConvertMng::ConvertSigned(m_awFlags[i]), (INT32)DISPARG_NOFREEVARIANT, "&"));
	...
	case VT_I4:
		{
			...
			i_strValue.Format("%i", nVal);
		}
		break;
	case VT_R4:
		{
			float fVal = i_pvarg->fltVal;
			i_strValue.Format("%f", fVal);
		}
		break;
	case VT_R8:
		{
			double dVal = i_pvarg->dblVal;
			i_strValue.Format("%f", dVal);
		}
		break;
	///////////////////////////////////////////////////////////////////
	// Date Type에 대해 추가
	case VT_DATE:
		{
			...
			i_strValue.Format("%04d-%02d-%02d", nYear, nMonth, nDay);
		}
		break;
	///////////////////////////////////////////////////////////////////
	case VT_BSTR:
...
BOOL CXlSimpleAutomation::SaveAsFile( LPCTSTR i_lpszFileName, CString i_strFileExt )
{
	...
	{
		if (i_strFileExt.CompareNoCase("xls") == 0)
		{
			eXlFormat = XLS;
		}
		else if (i_strFileExt.CompareNoCase("xlsx") == 0)
		{
			eXlFormat = XLSX;
		}
		else if (i_strFileExt.CompareNoCase("xlsm") == 0)
		{
			eXlFormat = XLSM;
		}
		else if (i_strFileExt.CompareNoCase("xlsb") == 0)
		{
			eXlFormat = XLSB;
		}
		else
